...
# describing a plane through one P, point and n, normal vector
# P = (px, py, pz)
# n = (nx, ny, nz)
# X - any point of the plane - will be perpendicular to the n normal vector
...
# boolean is3D
def VoronoiNeighbors(positions, is3D, d0max):
	# check weather is 2D or 3D
	if is3D :
		p =  [ n for (n,f) in positions ]	#component [0] is a 3d vector
		p += addGhosts( positions, d0max )
	else :	
		# kb az a kod ami most van
		p =  [ n[0][0:2] for n in positions ]
